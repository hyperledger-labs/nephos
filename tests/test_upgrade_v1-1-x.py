...
from nephos.upgrade_v11x import (
    extract_credentials,
    extract_crypto,
    main,
    upgrade_charts,
...
class TestExtractCredentials:
    ...
    def test_extract_credentials(
        self, mock_print, mock_secret_create, mock_secret_read
    ):
        secret_data = [
    ...
    @mock.patch("nephos.upgrade_v11x.secret_create")
    @mock.patch("nephos.upgrade_v11x.print")
    def test_extract_credentials_again(
        self, mock_print, mock_secret_create, mock_secret_read
    ):
...
class TestExtractCrypto:
    ...
    def test_extract_crypto(
        self, mock_get_pod, mock_print, mock_secret_create, mock_secret_read
    ):
        mock_pod_ex = mock.Mock()
        mock_pod_ex.execute.side_effect = [
            ("1", None),
            ("a-secret", None),  # signcerts
            ("1", None),
            ("a-secret", None),  # keystore
            ("1", None),
            ("a-secret", None),  # cacerts
        ...
        mock_get_pod.assert_has_calls([call("ord-namespace", "ord0", "hlf-ord")])
        mock_secret_read.assert_has_calls(
            [
        ...
        mock_print.assert_called_once_with(
            "Wrong number of files in intermediatecerts directory"
        )
        mock_secret_create.assert_has_calls(
            [
                call(
    ...
    @mock.patch("nephos.upgrade_v11x.print")
    @mock.patch("nephos.upgrade_v11x.get_pod")
    def test_extract_crypto_again(
        self, mock_get_pod, mock_print, mock_secret_create, mock_secret_read
    ):
        mock_pod_ex = mock.Mock()
        mock_get_pod.side_effect = [mock_pod_ex]
        extract_crypto(self.OPTS, "peer", verbose=True)
        mock_get_pod.assert_has_calls([call("peer-namespace", "peer0", "hlf-peer")])
        mock_secret_read.assert_has_calls(
            [
    ...
    @mock.patch("nephos.upgrade_v11x.print")
    @mock.patch("nephos.upgrade_v11x.get_pod")
    def test_extract_crypto_fail(
        self, mock_get_pod, mock_print, mock_secret_create, mock_secret_read
    ):
        mock_pod_ex = mock.Mock()
        mock_pod_ex.execute.side_effect = [("0", None)]  # signcerts
        mock_get_pod.side_effect = [mock_pod_ex]
        ...
        mock_pod_ex.execute.assert_called_once_with(
            "bash -c 'ls /var/hyperledger/msp/signcerts' | wc -l"
        )
        mock_print.assert_not_called()
        mock_secret_create.assert_not_called()
...
class TestUpgradeCharts:
    ...
    def test_upgrade_charts(
        ...
        mock_pod_ex = mock.Mock()
        mock_pod_ex.execute.side_effect = [("", None), ("", None)]  # ls  # cp
        mock_get_pod.side_effect = [mock_pod_ex]
    ...
    def test_upgrade_charts_again(
        ...
        mock_pod_ex = mock.Mock()
        mock_pod_ex.execute.side_effect = [("a-res", None)]  # ls
        mock_get_pod.side_effect = [mock_pod_ex]
        upgrade_charts(self.OPTS, "peer", verbose=True)
        mock_pod_ex.execute.assert_has_calls([call("ls /var/hyperledger/msp_old")])
