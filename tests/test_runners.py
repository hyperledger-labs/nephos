...
class TestRunnerComposer:
    ...
    def test_runner_composer(self, mock_deploy_composer, mock_install_network, mock_setup_admin):
...
class TestRunnerCrypto:
    ...
    def test_runner_crypto(self, mock_admin_msp, mock_channel_tx, mock_genesis_block, mock_setup_nodes):
        ...
        mock_genesis_block.assert_called_once_with(
            self.OPTS, verbose=False)
        mock_channel_tx.assert_called_once_with(
            self.OPTS, verbose=False)
        # Setup node MSPs
...
class TestRunnerFabric:
    ...
    def test_runner_fabric(self, mock_runner_ca, mock_runner_crypto, mock_runner_orderer, mock_runner_peer):
        runner_fabric(self.OPTS, upgrade=False)
        mock_runner_ca.assert_called_once_with(
            self.OPTS, upgrade=False, verbose=False)
        mock_runner_crypto.assert_called_once_with(
            self.OPTS, verbose=False)
...
class TestRunnerOrderer:
    ...
    def test_runner_orderer(self, mock_setup_ord):
        ...
        mock_setup_ord.assert_called_once_with(
            self.OPTS, upgrade=False, verbose=False)
...
class TestRunnerPeer:
    ...
    def test_runner_peer(self, mock_setup_channel, mock_setup_peer):
        ...
        mock_setup_peer.assert_called_once_with(
            self.OPTS, upgrade=False, verbose=False)
        mock_setup_channel.assert_called_once_with(
            self.OPTS, verbose=False)
