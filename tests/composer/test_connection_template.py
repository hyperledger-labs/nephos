...
class TestDefineOrderers:
    def test_define_orderers(self):
        res = define_orderers(['ord0', 'ord1'], ['ord0.local-cluster', 'ord1.local-cluster'])
        assert res == {
    ...
    def test_define_orderers_domain(self):
        res = define_orderers(['ord0', 'ord1'], ['ord0.local-cluster', 'ord1.local-cluster'], domain='a-domain.com')
        assert res == {
...
class TestDefinePeers:
    def test_define_peers(self):
        ...
        assert res == ({
                           'peer0': {'chaincodeQuery': True, 'endorsingPeer': True, 'eventSource': True,
                                     'ledgerQuery': True}
                       }, {
                           'peer0': {'url': 'grpc://peer0.local-cluster:7051',
                                     'eventUrl': 'grpc://peer0.local-cluster:7053'}})

    def test_define_peers_domain(self):
        res = define_peers(['peer0'], ['peer0.local-cluster'], 'an-org', domain='a-domain.com')
        assert res == ({
                           'peer0.an-org.a-domain.com': {'chaincodeQuery': True, 'endorsingPeer': True,
                                                         'eventSource': True, 'ledgerQuery': True}
                       }, {
                           'peer0.an-org.a-domain.com': {'url': 'grpc://peer0.local-cluster:7051',
                                                         'eventUrl': 'grpc://peer0.local-cluster:7053'}
                       })
...
class TestJsonCt:
    ...
    def test_json_ct(self, mock_define_orderers, mock_define_peers):
        ...
        res = json_ct(['peer0'], ['ord0'], ['peer0.local-cluster'], ['ord0.local-cluster'],
                      'a-ca', 'a-ca.a-domain.com', 'an-org', 'a-domain.com', 'AnMSP', 'a-channel')
